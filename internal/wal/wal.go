...
const (
	MetadataType walpb.EntryType = walpb.EntryType(iota)
	EntryType
	CrcType
...
func OpenForRead(lg *zap.Logger, dirpath string) (*WAL, error) {
	return open(lg, dirpath, false)
}

func open(lg *zap.Logger, dirpath string, write bool) (*WAL, error) {
	if lg == nil {
		lg = zap.NewNop()
	}
	names, err := getWALFiles(lg, dirpath)
	if err != nil {
		return nil, fmt.Errorf("[open] getWALFiles failed: %w", err)
	}

	rs, ls, closer, err := openWALFiles(lg, dirpath, names, 0, write)
	if err != nil {
		return nil, fmt.Errorf("[open] openWALFiles failed: %w", err)
	}
...
func getWALFiles(lg *zap.Logger, dirpath string) ([]string, error) {
	names, err := readWALNames(lg, dirpath)
	if err != nil {
		return nil, fmt.Errorf("readWALNames failed: %w", err)
	}

	if !isValidSeq(lg, names[0:]) {
		return nil, fmt.Errorf("wal: file sequence numbers (starting from %d) do not increase continuously", 0)
	}
...
func (w *WAL) ReadAll() (metadata []byte, ents []logpb.LogEntry, err error) {
	w.mu.Lock()
	defer w.mu.Unlock()
	...
	if w.decoder == nil {
		return nil, nil, ErrDecoderNotFound
	}
	decoder := w.decoder
...
func (w *WAL) saveEntry(e *logpb.LogEntry) error {
...
func (w *WAL) Save(ents []logpb.LogEntry, mustSync bool) error {
	w.mu.Lock()
	defer w.mu.Unlock()
